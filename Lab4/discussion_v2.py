...
class SpatialCANN(bp.DynamicalSystem):
  ...
  def couple_update(self):
    ...
    # Ipg = bm.dot(Wpg1, self.P.r) + bm.dot(Wpg2, self.P.u) + bm.dot(Wpg3, self.P.u)
    # Igp1 = bm.dot(Wpg1.T, self.G1.r)
    # Igp2 = bm.dot(Wpg2.T, self.G2.r)
    # Igp3 = bm.dot(Wpg3.T, self.G3.r)
    ...
    # print shape
    # print("Ipg shape:", Ipg.shape)
    # print("Igp1 shape:", Igp1.shape)
    # print("Igp2 shape:", Igp2.shape)
    # print("Igp3 shape:", Igp3.shape)
  ...
  def update(self,pos=None):
    ...
    return self.P_u, self.G1_u, self.G2_u, self.G3_u, self.P.decode()
...
with bm.environment(dt=0.1):
  ...
  for i in range(num_2):
    ...
    assert pos < PGcann.L
    position[i+num_1] = pos
  for i in range(num_3):
    pos = position[i+num_1+num_2-1] - v_ext * bm.dt
    assert pos > -PGcann.L
    position[i+num_1+num_2] = pos
  position = position.reshape(-1,1)
  
  uP, uG1, uG2, uG3, decoded_positions = bm.for_loop(run, (indices, position))
...
# Save the plot in the current working directory
  # plt.savefig(f'True_vs_Decoded_Position_noise_{params["P_CANN1D"]["noise"]}.png')
...
#   decoded_positions = np.array(decoded_positions)
#   plt.plot(decoded_positions, label=f'Noise Level {noise:.1f}')

# plt.xlabel('Time Step')
# plt.ylabel('Position')
# plt.legend()
# plt.title('Decoded Position with Varying Noise Levels')
# plt.show()

# bp.visualize.animate_1D(
#     dynamical_vars=[{'ys': uP, 'xs': PGcann.P.x, 'legend': 'u'},
#                     {'ys': PGcann.P.e_input, 'xs': PGcann.P.x, 'legend': 'Iext'},
#                     {'ys': PGcann.P.g_input, 'xs': PGcann.P.x, 'legend': 'Ig_sum'},],
#     frame_step=1,
#     frame_delay=40,
#     show=True,
#     save_path=f'P_CANN.gif',
# )

# bp.visualize.animate_1D(
#     dynamical_vars=[{'ys': uG1, 'xs': PGcann.G1.x, 'legend': 'u'},
#                     {'ys': PGcann.G1.e_input, 'xs': PGcann.G1.x, 'legend': 'Iext'},
#                     {'ys': PGcann.G1.p_input, 'xs': PGcann.G1.x, 'legend': 'Ip_sum'},],
#     frame_step=1,
#     frame_delay=40,
#     show=True,
#     save_path=f'G1_CANN.gif',
# )
...
#     frame_delay=40,
#     show=True,
#     save_path=f'G1_CANN_{CUE}.gif',
# )

# bp.visualize.animate_1D(
#     dynamical_vars=[{'ys': uG2, 'xs': PGcann.G2.x, 'legend': 'u'},],
#     frame_step=1,
#     frame_delay=40,
#     show=True,
#     save_path=f'G2_CANN_{CUE}.gif',
# )

# bp.visualize.animate_1D(
#     dynamical_vars=[{'ys': uG3, 'xs': PGcann.G3.x, 'legend': 'u'},],
#     frame_step=1,
#     frame_delay=40,
#     show=True,
#     save_path=f'G3_CANN_{CUE}.gif',
# ) 
